#urllib的request模块可以非常方便地抓取URL内容，也就是发送一个GET请求到指定页面，然后返回HTTP的响应
#例如，对豆瓣的一个URL抓取，并返回响应
...
#模拟浏览器发送GET请求，就需要使用Request对象，通过Request对象添加HTTP头，就可以把请求伪装成浏览器
from urllib import request
...
#模拟一个微博登录，先读取登录的邮箱和口令，然后按照微博的登录页格式以username=xxx&password=xxx的编码传入
